...
class DiagnosticoDownloadMidias:
    ...
    def tarefa_1_analisar_fluxo_atual(self):
        ...
        try:
            from multichat_system.webhook.views import download_media_via_wapi
        ...
        try:
            from multichat_system.webhook.views import process_media_automatically
            ...
            try:
                ...
                for tipo in tipos_midia:
                    if tipo in msg_content:
                        ...
                        print(f"✅ Webhook {webhook.id}: {tipo} detectado")
                        ...
                        if not (tem_media_key and tem_direct_path and tem_mimetype):
                            self.adicionar_problema("DADOS", f"Webhook {webhook.id} com dados incompletos para {tipo}")
                        
                        break
            except Exception as e:
                print(f"❌ Erro ao analisar webhook {webhook.id}: {e}")
    ...
    def tarefa_3_verificar_autenticacao_multicliente(self):
        ...
        media_storage_base = Path("multichat_system/media_storage")
    ...
    def tarefa_4_comparar_testes_vs_automatico(self):
        ...
        if webhook_real:
            try:
                data = json.loads(webhook_real.raw_data)
